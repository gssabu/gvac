...
const  Encoder::ENC_Hardware_t Encoder::hardware[] = {	
...
#endif
const uint8_t Encoder::_hardware_count =  (sizeof(Encoder::hardware)/sizeof(Encoder::hardware[0]));

Encoder * Encoder::list[5];

  uint8_t Encoder::compareValueFlag;
 uint32_t Encoder::homeCounter;
 uint32_t Encoder::indexCounter;

 
void Encoder::setInitConfig() {
...
Encoder::Encoder(uint8_t encoder_ch, uint8_t PhaseA_pin, uint8_t PhaseB_pin, uint8_t pin_pus, uint8_t index_pin, uint8_t home_pin, uint8_t trigger_pin){
	if(encoder_ch >= 0){
...
void Encoder::getConfig1(enc_config_t *config)
{
...
void Encoder::printConfig(enc_config_t *config)
{
...
void Encoder::setConfigInitialPosition()
{
...
void Encoder::Init(const enc_config_t *config)
{
    uint32_t tmp16;
...
uint32_t Encoder::getHoldPosition()
{
    uint32_t ret32;
...
uint16_t Encoder::getPositionDifference()
{
    return channel[_encoder_ch].ENC->POSD;
}


uint16_t Encoder::getHoldDifference()
{
    return channel[_encoder_ch].ENC->POSDH;
}


uint16_t Encoder::getRevolution()
{
    return channel[_encoder_ch].ENC->REV;
}

uint16_t Encoder::getHoldRevolution()
{
    return channel[_encoder_ch].ENC->REVH;
}

void Encoder::enc_xbara_mapping(uint8_t pin, uint8_t PHASE, uint8_t PUS){ 
...
void Encoder::enableInterrupts(const enc_config_t *config)
{
    uint32_t tmp16 = 0;
...
void Encoder::disableInterrupts(uint32_t flag)
{
    uint16_t tmp16 = 0U;
...
void Encoder::clearStatusFlags(uint32_t flag, uint8_t index)
{
    uint32_t tmp16 = 0U;
...
inline void Encoder::checkAndProcessInterrupt(uint8_t index) 
{
...
void Encoder::isr(uint8_t index) 
{	
...
void Encoder::resetCompareInterupt()
{
